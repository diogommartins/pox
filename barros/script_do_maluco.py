...
def _get_tag(src, dst):
    """
    Gets a tag between a Source Switch and a Destination Switch
    """

    tag = spf_table.get((src, dst))

    possible_tags = [40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96,
                     100]  # TODO improve multiples of TOS from 40 to 256

    if tag == None:  # There's no tag associated to that src -> dst
        if src == dst:
            tag = None
        else:  # TODO improve this feature
...
class Switch(EventMixin):
    ...
    def _install_pathflow_without_tag(self, switch, out_port, match, buf=None):
        print('cheguei em path flow withou tag')
        msg = of.ofp_flow_mod()
    ...
    def _install_pathflow_push_tag(self, switch, tag, out_port, match, buf=None):
        print('cheguei em path flow push tag')
        msg = of.ofp_flow_mod()
        msg.match.dl_dst = match.dl_dst
        msg.idle_timeout = FLOW_IDLE_TIMEOUT
        msg.hard_timeout = FLOW_HARD_TIMEOUT
    ...
    def _install_pathflow_just_tag(self, switch, tag, out_port, match, buf=None):
        print(
            'cheguei em path flow just tag, self:', self, 'switch:', switch, 'tag:', tag, 'out_port:', out_port, 'match',
            match)
        msg = of.ofp_flow_mod()
        msg.match.dl_type = match.dl_type  # todo cara isso eh primordial
    ...
    def _install_pathflow_pop_tag(self, switch, out_port, match, buf=None):
        print('cheguei em path flow pop tag')
        msg = of.ofp_flow_mod()
        msg.match.dl_dst = match.dl_dst
        msg.idle_timeout = FLOW_IDLE_TIMEOUT
        msg.hard_timeout = FLOW_HARD_TIMEOUT
        msg.actions.append(of.ofp_action_nw_tos(nw_tos=0))  # TODO Improve this to pop the tag
        msg.actions.append(of.ofp_action_output(port=out_port))
        msg.buffer_id = buf
        switch.connection.send(msg)

    def _remove_pathflow(self, mac):  # TODO isso eh temporario
        print ('cheguei em remove path flow, self:', self, 'mac:', mac)
        msg = of.ofp_flow_mod()
    ...
    def _install_path_pathflow(self, tag, p, match, packet_in=None):
        print(
            '_install pathflow path cara eu preciso instalar esse caminho faz isso para mim por favor seguem os parametros')
        print(
            'self sai alguma coisa', self, 'tag:', tag, 'p:', p, 'match.dl_dst', match.dl_dst, 'packet_in:', packet_in)

        wp = WaitingPath(p, packet_in)  # TODO  Revisar

        dst_sw = mac_map.get(match.dl_dst)[0]
        print ('dst_sw :', dst_sw)

        if len(p) == 1:  # "local" traffic
            for sw, in_port, out_port in p:
                print ('unico switch')
                self._install_pathflow_without_tag(sw, out_port, match)
                msg = of.ofp_barrier_request()
                sw.connection.send(msg)
                wp.add_xid(sw.dpid, msg.xid)
        if len(p) >= 2:
            for sw, in_port, out_port in p:
                if sw == self:
                    print ('primeiro switch')
                    self._install_pathflow_push_tag(sw, tag, out_port, match)
                    msg = of.ofp_barrier_request()
                    sw.connection.send(msg)
                    wp.add_xid(sw.dpid, msg.xid)
                if (sw != self) and (sw != dst_sw):
                    print ('sw intermediario')
                    if (match.dl_type == 0x800):
                        print ('entrei dentro de dl type = 0 x 800 o pacote eh ip')
                        self._install_pathflow_just_tag(sw, tag, out_port, match)
                        msg = of.ofp_barrier_request()
                        sw.connection.send(msg)
                        wp.add_xid(sw.dpid, msg.xid)
                    else:
                        print ('entrou na excecao : nao eh ip')
                        self._install(sw, in_port, out_port, match)
                        msg = of.ofp_barrier_request()
                        sw.connection.send(msg)
                        wp.add_xid(sw.dpid, msg.xid)
                if sw == dst_sw:
                    print ('ultimo switch')
                    self._install_pathflow_pop_tag(sw, out_port, match)
                    msg = of.ofp_barrier_request()
                    sw.connection.send(msg)
                    wp.add_xid(sw.dpid, msg.xid)
    ...
    """
        ...
        print ('tag dentro de install:', tag)

        self._install_path_pathflow(tag, p, match, event.ofp)

        '''log.debug("Installing path for %s -> %s %04x (%i hops)",         #TODO atencao esse eh o passo principal do software principal
    ...
    def _handle_PortStatus(self, event):
        print ('cara eu estou em PortSatus - self:', self, 'event:', event)
        '''print ('event.connection: ',event.connection)
      print ('event.dpid:',event.dpid)
      print ('event.port:', event.port)
      print ('event.ofp:',event.ofp)
      print ('event.modified:', event.modified)
      print ('event.added:', event.added)
      print ('event.deleted :', event.deleted)'''

        if event.deleted == True:  # TODO isso eh temporario
            # Some Host could have gone down
            print ('entrei em event.deleted=true')

            lValue = (self, event.port)
            print ('lvalue:', lValue)
            mac = [key for key, value in mac_map.iteritems() if value == lValue][0]
            print ('mac:', mac)
            self._remove_pathflow(mac)
            global moving_mac
            moving_mac = mac
            print ('moving mac dentro de event deleted:', moving_mac)

        if event.added == True:  # TODO isso eh temporario
            print ('cheguei em event.addeed')
            global moving_mac
            mac = moving_mac
            print ('mac   deixa eu ver se ainda se tem a informacao do mac viajante:', mac)

            mac_map[mac] = (self, event.port)
...
class l2_multi(EventMixin):
    _eventMixin_events = {PathInstalled}
